...
app.get('/api/students', async (req, res) => {
    try {
        // Optimization: Don't join details for list view
        const students = await db.findAll('students', { join: false });
        res.json({ students });
    } catch (error) {
        console.error(error);
...
// GET: Lesson Memos for a student
app.get('/api/students/:id/lesson-memos', async (req, res) => {
    try {
        if (db.getLessonMemos) {
            const memos = await db.getLessonMemos(req.params.id);
            res.json(memos);
        } else {
            res.json({});
        }
...
// GET: Memo History for a student
app.get('/api/students/:id/memo-history', async (req, res) => {
    try {
        if (db.getMemoHistory) {
            const history = await db.getMemoHistory(req.params.id);
            res.json(history);
        } else {
            res.json([]);
        }
